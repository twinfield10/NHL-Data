...
# 1) Load Rosters From Historical Team Links
def historical_roster_load():
    ...
    goalies_slim = slim_df.filter(pl.col('pos_G') == 1)
...
#    """This function will load all rosters possible from the current season and
#    find players using individual game logs rather than the current roster"""
#
#    # Load PBP GameID's With Missing Player ID Data (only event_player_1_id)
#    for i in range(2012, 2023):
#        df = (
#            pl.read_parquet(f"PBP/parquet/API_RAW_PBP_Data_{i}.parquet")
#            .join(roster_slim.select(['id', 'shootsCatches']), left_on = 'event_player_1_id', right_on = 'id', how = 'left')
#        )
#        missing_df = (
#            df
#            .filter((pl.col('shootsCatches').is_null()) & (~pl.col('event_player_1_id').is_null()))
#            .select('game_id', 'event_player_1_id')
#        )
#        print(f"{i} NHL Season contains {missing_df.select('event_player_1_id').unique().height} missing player IDs from {missing_df.select('game_id').unique().height} games")
#
...
# Helper: Create Normalized Schema
raw_schema = {
    'id': 'i32',
    'gameDate': 'str',
    'season': 'i32',
...
# 1) LOAD RAW: Connect To NHL API To Get Game Data
def ping_nhl_api(i):
...
# 2) CLEAN RAW: Normalize Schema of Play by Play Data
...
# 3) Append Shift Data To Play By Play
...
def get_game_ids(start = '2010-10-07', end = max_load_date):
    ...
    INPUTS:
    season_start = Earliest year of season (i.e., 2010-2011 Season is 2010)
    season_end = Latest year of season (i.e., 2023-2024 Season is 2024)"""

    # 1) Get Game ID's From Schedule
    season_ids = []
    for i in range(2010, 2024):
        season_ids.append(
            pl.read_parquet(f"Schedule/parquet/NHL_Schedule_{str(i)+str(i+1)}.parquet")
            .filter((pl.col('game_date') >= start) & (pl.col('game_date') <= end))['game_id']
            )

    game_ids = []
    for series in season_ids:
        game_ids.extend(series.to_list())
...
def load_all_pbp(season = 2023):
    ...
    for i in game_ids:
        ...
        if (str(i)[-3:] == "500") | (str(i)[-3:] == "000"):
            print(f"LOAD UPDATE: Game {i} took {round(shift_elap,2)} | Each game is taking ~{round(average_shift_time,2)} Seconds | For {n_games} Games It will Take {round(hour_pace,2)} Hours")
